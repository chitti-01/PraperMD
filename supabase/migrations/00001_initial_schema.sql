-- ============================================================
-- PaperMD Migration 00001: Initial Core Schema & Seed Data
-- ============================================================
...
CREATE TABLE IF NOT EXISTS question_papers (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT check_qp_status CHECK (status IN ('active', 'removed')),
  CONSTRAINT check_qp_mbbs_year CHECK (mbbs_year IN ('1st MBBS', '2nd MBBS', '3rd MBBS', 'Final MBBS')),
  CONSTRAINT check_qp_exam_attempt CHECK (exam_attempt IN ('Main Examination', 'Supplementary Examination')),
  CONSTRAINT check_qp_file_type CHECK (file_type IN ('pdf', 'image', 'multi_image'))
);
...
CREATE TABLE IF NOT EXISTS reports (
  ...
  CONSTRAINT check_reports_status CHECK (status IN ('open', 'resolved', 'dismissed'))
);
...
-- 1. Colleges RLS
DROP POLICY IF EXISTS "Public students can view active colleges" ON colleges;
CREATE POLICY "Public students can view active colleges"
  ON colleges FOR SELECT
  USING (is_active = true);

-- 2. Subjects RLS
DROP POLICY IF EXISTS "Public students can view active subjects" ON subjects;
CREATE POLICY "Public students can view active subjects"
  ON subjects FOR SELECT
  USING (is_active = true);

-- 3. Exam Types RLS
DROP POLICY IF EXISTS "Public students can view active exam types" ON exam_types;
CREATE POLICY "Public students can view active exam types"
  ON exam_types FOR SELECT
  USING (is_active = true);

-- 4. Question Papers RLS
DROP POLICY IF EXISTS "Public students can view active question papers" ON question_papers;
CREATE POLICY "Public students can view active question papers"
  ON question_papers FOR SELECT
  USING (status = 'active');

-- 5. Reports RLS (Public students can submit reports; viewing restricted to service role/admin)
DROP POLICY IF EXISTS "Public students can read open reports" ON reports;
DROP POLICY IF EXISTS "Public students can submit reports" ON reports;
DROP POLICY IF EXISTS "Only service role can view reports" ON reports;

CREATE POLICY "Public students can submit reports"
